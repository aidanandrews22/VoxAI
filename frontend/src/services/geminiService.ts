...
export async function streamChatWithGemini(
  ...
  try {
    ...
    if (lastMessage.content.includes('data: {"type":')) {
      console.error('Message appears to be in SSE format. This is likely an error.');
      throw new Error('Invalid message format. Cannot send an SSE response as a message.');
    }
