...
export default function NotebooksPage() {
  ...
  useEffect(() => {
    if (supabaseUserId && selectedFolderId && selectedFolderId !== 'unorganized') {
      refreshFolders();
    }
  ...
  const handleCreateNotebook = async (notebookData: {
    ...
    try {
      ...
      const folderId = notebookData.folderId !== 'unorganized' ? notebookData.folderId : undefined;
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (getCurrentFolderId(editingNotebook.id) !== notebookData.folderId) {
          ...
          if (selectedFolderId && selectedFolderId !== 'unorganized' && selectedFolderId !== notebookData.folderId) {
            setNotebooks(notebooks.filter(nb => nb.id !== editingNotebook.id));
          }
        }
  ...
  const showNestedToggle = selectedFolderId !== null && 
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
                ...
                { notebooks.length !== 0 && (
                  ...
                  >
                    ...
                    <div className="text-center">
                      <p className="text-gray-500 dark:text-gray-400 font-medium">Create New Notebook</p>
                    </div>
                  </div>
                )}
      ...
      <NotebookModal
        ...
        isEditing={isEditing}
        selectedFolderId={selectedFolderId !== 'unorganized' ? selectedFolderId : undefined}
      />
    </div>
  );
}